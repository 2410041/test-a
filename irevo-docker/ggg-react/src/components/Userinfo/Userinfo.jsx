...
function Userinfo({ viewUser = null }) {
    ...
    useEffect(() => {
        ...
        const fetchPersonality = async () => {
            try {
                ...
                console.log('[Userinfo] chart GET response:', res.data);
                ...
                if (item.chart_score !== undefined && item.chart_score !== null) {
                    if (Array.isArray(item.chart_score)) {
                        ...
                        try {
                            ...
                            console.warn('[Userinfo] chart_score parse failed:', e);
                        }
    ...
    return (
        <div className="ui-mypage-root">
            {/* プロフィールカード */}
            <div className='ui-card-wrap'>
                <div className="ui-card ui-profile-card">
                    <div className="ui-icon">
                        <img src="./images/gyo.JPG" alt="" className="ui-img" />
                        <table className="ui-table">
                            <tbody>
                                <tr><td className="ui-nick">ニックネーム</td></tr>
                                <tr>
                                    <td className="ui-user-name">
                                        {user?.u_nick}
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <strong className='ui-mytype'>自己分析結果</strong>
                    <div className='ui-container4'>
                        {loadingChart ? (
                            <p>読み込み中...</p>
                        ) : personalityType ? (
                            <div className={`ui-type-result ${personalityType ? personalityType.toLowerCase() : ''}`}>
                ...
                {/* 資格カード */}
                <div className="ui-card ui-qualification-card">
                    <h2 className="ui-qu-title">資格・スキル</h2>
                    <ul className="ui-qu-ul">
                        <li>TOEIC 800</li>
                        <li>基本情報技術者</li>
                        <li>日商簿記2級</li>
                    </ul>
                ...
                {/* タブカード */}
                <div className="ui-card ui-tab-card">
                    <Tabnav user={user} />
                </div>
            </div>
        </div>
